 ...
 * @date 2020/04/05
 **/
public abstract class BaseFilterGroup<T, S extends BaseFilterGroup<T, S>> extends FilterGroupDescriptor {
    ...
    public S and(boolean enable, UnaryOperator<FilterGroup<T>> groupConsumer) {
        if (enable) {
            FilterGroup<T> baseFilterGroup = new FilterGroup<>();
            baseFilterGroup.setCondition(FilterCondition.AND);
            this.addFilters(groupConsumer.apply(baseFilterGroup));
        }
        return (S) this;
    }
    ...
    public S or(boolean enable, UnaryOperator<FilterGroup<T>> groupConsumer) {
        if (enable) {
            FilterGroup<T> baseFilterGroup = new FilterGroup<>();
            baseFilterGroup.setCondition(FilterCondition.OR);
            this.addFilters(groupConsumer.apply(baseFilterGroup));
        }
        return (S) this;
    }
